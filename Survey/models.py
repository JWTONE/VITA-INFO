...
class SurveyInfo(models.Model):
    ...
    exercise_frequency_per_week = models.Choices(choices=EXERCISE_CHOICES, max_lenght=1)
    average_sleep_hours_per_day = models.Choices(choices=SLEEP_CHOICES, max_lenght=1)
    smoking_status = models.Choices(choices=SMOKE_CHOICES, max_lenght=1)
    alcohol_consumption = models.Choices(choices=ALCOHOL_CHOICES, max_lenght=1)
    average_meals_per_day = models.Choices(choices=EAT_CHOICES, max_lenght=1)
    main_foods = models.Choices(choices=DIET_CHOICES, max_lenght=1)
    snacks = models.CharField(choices=SNACK_CHOICES, max_lenght=100, default='과일')
    snacks_other = models.CharField(max_length=100)
    health_goals = models.CharField(choices=HEALTHGOAL_CHOICES, max_lenght=100, default='체중감량')
    health_goals_other = models.CharField(max_length=100)
    interested_supplements = models.CharField(choices=INTERESTED_CHOICES, max_lenght=100, default='비타민')
